...
export default function page({params}:{params:Promise<{reportId:string}>}){
  ...
  const fetchReviewReport = async () => {
    const res = await reviewReportApi.admGetReviewReport(reportId)
    if (res == null){
      return
    }
    setSelectedReport(res)
    console.log(res.reportState)
  ...
  const openMessageDialog = (type: 'ACCEPT' | 'REJECT' | 'EDIT_REQUIRED' | 'DELETE') => {
    setActionType(type);
        ...
        <CardContent className="space-y-6">
          ...
          {selectedReport.reportState === 'PENDING' && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4">
              <Button 
                onClick={() => openMessageDialog('ACCEPT')}
                variant="outline"
              ...
              <Button 
                onClick={() => openMessageDialog('REJECT')}
                variant="outline"
              ...
              <Button 
                onClick={() => openMessageDialog('EDIT_REQUIRED')}
                variant="outline"
              ...
              <Button 
                onClick={() => openMessageDialog('DELETE')}
                variant="destructive"
          ...
          <Dialog open={isMessageDialogOpen} onOpenChange={setIsMessageDialogOpen}>
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle>
                  {actionType === 'ACCEPT' ? '리뷰 승인 처리' : 
                    actionType === 'REJECT' ? '신고 기각 처리' :
                    actionType === 'EDIT_REQUIRED' ? '수정 요청 처리' : '리뷰 삭제 처리'}
                </DialogTitle>
                <DialogDescription>
                  {actionType === 'ACCEPT' 
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <Button 
                    ...
                    disabled={(actionType === 'DELETE' || actionType === 'EDIT_REQUIRED') && !adminMessage.trim()}
                  >
                    {actionType === 'ACCEPT' ? '승인하기' : 
                      actionType === 'REJECT' ? '신고 기각하기' :
                      actionType === 'EDIT_REQUIRED' ? '수정 요청하기' : '삭제하기'}
                  </Button>
                </div>
              </div>
            </DialogContent>
